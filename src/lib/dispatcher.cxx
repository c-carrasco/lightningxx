...
class Dispatch {
  ...
  public:
    Dispatch (
      const Dispatcher::Snapshot &snapshot,
      const Dispatcher::Snapshots &snapshots,
      HttpRequest &request,
      HttpResponse &response,
      std::function<void()> fallthrough = {},
      std::function<void (std::exception_ptr)> propagate = {},
      ...
      _request { request },
      _response { response },
      _inheritedParams { request.params },
      _allowed { std::move (allowed) },
      _fallthrough { std::move (fallthrough) },
      _propagate { std::move (propagate) },
      _async { async },
    ...
    static Task<> runAsync (
      std::shared_ptr<const Dispatcher::Snapshots> snapshots,
    ...
    void run (size_t layerIndex = 0) {
      ...
      while (layerIndex < _snapshot.layers.size()) {
        ...
        try {
          if (
            (_request.method == HttpMethod::kOptions) &&
            (layer.method != HttpMethod::kUnknown) &&
  ...
  private:
    ...
    void allow (HttpMethod method) {
      ...
      if (method == HttpMethod::kGet)
        (*_allowed)[static_cast<size_t> (HttpMethod::kHead)] = true;
    }
    ...
    bool options() {
      ...
      if (std::none_of (_allowed->begin(), _allowed->end(), [] (bool present) { return present; }))
        return false;
    ...
    void runError (std::exception_ptr error) {
      ...
      if (index == _snapshot.errors.size()) {
        ...
        catch (const RouteDecodeError &) {
          _response.status (400).send ("Bad request");
        }
        catch (const RequestParseError &failure) {
          _response.status (failure.status()).send (failure.what());
        }
        catch (...) {
...
void Dispatcher::addRoute (HttpMethod method, std::string_view path, std::vector<Middleware> handlers) {
  if (
    (static_cast<size_t> (method) >= static_cast<size_t> (kNumHttpMethods)) ||
    handlers.empty() ||
...
void Dispatcher::mount (std::string_view prefix, std::shared_ptr<Dispatcher> router) {
  if (!router)
    throw std::invalid_argument ("Empty router");
  ...
  if (reachesThis (router.get()))
    throw std::invalid_argument ("Router mounts cannot form cycles");

  auto layer = std::make_shared<const Layer> (Layer {
    HttpMethod::kUnknown, std::move (pattern), {}, std::move (router)
  });
